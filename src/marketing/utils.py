...
MAILCHIMP_DATA_CENTEER = getattr(settings, "MAILCHIMP_DATA_CENTEER", None)
...
class Mailchimp(object):
    ...
    def change_subscription_status(self, email, status='unsubscribed'):
        ...
        end_point = self.get_members_endpoint() + "/" + hashed_email
        data = {
            "email_address": email,
            "status": self.check_valid_status(status),
        }
        r = requests.put(end_point, auth=("", self.key), data=json.dumps(data))
    ...
    def add_email(self, email):
        ...
        # return r.json()
        # change_subscription_status가 put을 사용했으므로 가능함.
